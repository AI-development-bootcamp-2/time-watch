...
// Also accept the English values so the route is forgiving.
const LOCATION_MAP = {
  'משרד': 'office',
  'לקוח': 'client_site',
  'בית': 'home',
...
// Resolve a task id from the form values. Tries (project + task) name first,
// then falls back to a task-name-only lookup. Returns null if not found.
async function findTaskId(taskName, projectName, conn = db) {
...
async function isMonthLocked(isoDate, conn = db) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingTotal = existingRows.reduce((s, r) => s + Number(r.duration_hours), 0)
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(rows.map(r => ({ ...r, date: dateToIso(r.date) })))
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (await isMonthLocked(dateToIso(existing.date))) {
      return res.status(423).json({ error: 'This month is locked and cannot be modified' })
    }
    ...
    const start = (patch.start_time || existing.start_time || '').slice(0, 5)
    const end = (patch.end_time || existing.end_time || '').slice(0, 5)
