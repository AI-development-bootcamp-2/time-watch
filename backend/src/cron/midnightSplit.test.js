...
const { runMidnightSplit } = require('./midnightSplit')
...
/**
 * Build a fake knex callable that:
 *   - Returns a query-builder stub when called as fakeKnex('timer_state')
 *   - Exposes fakeKnex.raw() so the function can embed raw SQL fragments
 *   - Records every .where().select() and .where().update() call so tests
 *     can assert on them.
 *
 * @param {object[]} selectResult  Rows to return from .select()
 * @returns {{ db: Function, mocks: object }}
 */
function buildFakeKnex(selectResult) {
  const mocks = {
